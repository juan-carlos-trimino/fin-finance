...
type WfOaEppPages interface {
  OaEppPages(http.ResponseWriter, *http.Request)
}
...
func NewWfOaEppPages() WfOaEppPages {
  return &wfOaEppPages {
    currentPage: "rhs-ui1",
    ...
    if strings.EqualFold(p.currentPage, "rhs-ui1") {
      ...
      if req.Method == http.MethodPost {
        ...
        if n, err = strconv.ParseFloat(p.fd1N, 64); err != nil {
          ...
          p.fd1Result = fmt.Sprintf("Payment: $%.2f", oa.O_Payment_FV(fv, i / 100.0,
      ...
      ***/
      ...
      if req.Method == http.MethodPost {
        ...
        if n, err = strconv.ParseFloat(p.fd2N, 64); err != nil {
          ...
          var oa finances.Annuities
          p.fd2Result = fmt.Sprintf("Payment: $%.2f", oa.O_Payment_PV(pv, i / 100.0,
