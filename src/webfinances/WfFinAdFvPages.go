    ...
    if strings.EqualFold(currentRHS, "rhs-ui2") {
      ...
      if req.Method == http.MethodPost {
        ...
        if n, err = strconv.ParseFloat(af.Fd2N, 64); err != nil {
          ...
          var oa finances.Annuities
          af.Fd2Result = fmt.Sprintf("Future Value: $%.5f",
            oa.D_FutureValue_PMT(pmt, i / 100.0, oa.GetCompoundingPeriod(af.Fd2Compound[0], true),
            n, oa.GetTimePeriod(af.Fd2TimePeriod[0], true)))
        }
        logger.LogInfo(fmt.Sprintf("n = %s, tp = %s, interest = %s, cp = %s, pmt = %s, %s", af.Fd2N,
          af.Fd2TimePeriod, af.Fd2Interest, af.Fd2Compound, af.Fd2PMT, af.Fd2Result), correlationId)
      }
      ...
      renderer.Render(res, "layout", templatesNeeded, renderer.PageData{
        Data: struct{
          ...
          Fd2PMT string
          Fd2Result string
        } { "Annuity Due / Future Value", logger.DatetimeFormat(), financesMenuPage, af.CurrentButton, newSession.CsrfToken,
