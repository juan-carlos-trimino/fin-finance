...
type WfAdFvPages interface {
  AdFvPages(http.ResponseWriter, *http.Request)
}
...
func NewWfAdFvPages() WfAdFvPages {
  return &wfAdFvPages {
    currentPage: "rhs-ui2",
...
func (p *wfAdFvPages) AdFvPages(res http.ResponseWriter, req *http.Request) {
  ...
  logEntry.Print(INFO, correlationId, []string {
    "Entering AdFvPages/webfinances.",
  })
    ...
    /*} else*/ if strings.EqualFold(p.currentPage, "rhs-ui2") {
      ...
      if req.Method == http.MethodPost {
        ...
        if n, err = strconv.ParseFloat(p.fd2N, 64); err != nil {
          ...
          var oa finances.Annuities
          p.fd2Result = fmt.Sprintf("Future Value: $%.2f", oa.D_FutureValue_PMT(pmt, i / 100.0,
